...
from relational_path.graph_sampler import *
...
class SubgraphDataset(Dataset):
    ...
    def __init__(self, db_path, db_name_pos, db_name_neg, raw_data_paths, included_relations=None, add_traspose_rels=False, num_neg_samples_per_link=1, use_kge_embeddings=False, dataset='', kge_model='', file_name=''):
        # create db file
        self.main_env = lmdb.open(db_path, readonly=True, max_dbs=3, lock=False, map_size=int(1e9))
        # create corresponding db files
        ...
        # Add transpose matrices to handle both directions of relations. 有向图-> 无向图
    ...
    def __getitem__(self, index):   # 当访问不存在的属性时调用，index = 0，batch 时会更新
        with self.main_env.begin(db=self.db_pos) as txn:
        ...
        # path = obtain_dglpaths(subgraph_pos, 0, 1, 4)
        # all_rel_paths = obtain_dgl_relation_paths(path, subgraph_pos)
    ...
    def _prepare_subgraphs(self, nodes, r_label, n_labels):
        subgraph = dgl.DGLGraph(self.graph.subgraph(nodes))  # 将nodes构成的子图转换为dgl图
        subgraph.edata['type'] = self.graph.edata['type'][self.graph.subgraph(nodes).parent_eid]  # 子图中对应边是什么
        subgraph.edata['label'] = torch.tensor(r_label * np.ones(subgraph.edata['type'].shape), dtype=torch.long)  # r_label

        edges_btw_roots = subgraph.edge_id(0, 1)    # roots间的边
        rel_link = np.nonzero(subgraph.edata['type'][edges_btw_roots] == r_label)   # 0,1之间是否为r_label
        ...
        kge_nodes = [self.kge_entity2id[self.id2entity[n]] for n in nodes] if self.kge_entity2id else None  # 默认None
        n_feats = self.node_features[kge_nodes] if self.node_features is not None else None     # 默认None
    ...
    def _prepare_features_new(self, subgraph, n_labels, n_feats=None):
        ...
        label_feats[np.arange(n_nodes), n_labels[:, 0]] = 1     # d(i,u)
        label_feats[np.arange(n_nodes), self.max_n_label[0] + 1 + n_labels[:, 1]] = 1       # d(i,v)
        # label_feats = np.zeros((n_nodes, self.max_n_label[0] + 1 + self.max_n_label[1] + 1))
