...
class RGCNBasisLayer(RGCNLayer):
    def __init__(self, inp_dim, out_dim, aggregator, attn_rel_emb_dim, num_rels, num_bases=-1, bias=None,
                 activation=None, dropout=0.0, edge_dropout=0.0, is_input_layer=False, has_attn=False, self_attn=False):
        super(
            RGCNBasisLayer,
            self).__init__(
            inp_dim,
        ...
        if self.self_attn:
            self.A = nn.Linear(self.inp_dim + self.attn_rel_emb_dim, inp_dim)
            self.B = nn.Linear(2 * self.inp_dim + self.attn_rel_emb_dim, 1)
        else:
            if self.has_attn:
    ...
    def propagate(self, g, attn_rel_emb=None):
        ...
        def msg_func(edges):
            ...
            if self.self_attn:
                e = torch.cat([edges.src[input_], attn_rel_emb(edges.data['type'])], dim=1)
                e_linear = F.relu(self.A(e))
                rel_emb = attn_rel_emb(edges.data['label'])
                s = torch.cat([e_linear, rel_emb, edges.dst[input_]], dim=1)
                a = torch.sigmoid(self.B(s))
            else:
                if self.has_attn:
