...
from relational_path.path_process import path_generate, path_emb_generate_batch_ori
    ...
    def eval(self, save=False):
        ...
        with torch.no_grad():
            for b_idx, batch in enumerate(dataloader):
                ...
                pos_paths_emb_batch, s_p_pos = path_emb_generate_batch_ori(pos_paths, rels_emb.cpu(), target_rels.cpu())
                neg_paths_emb_batch, s_p_neg = path_emb_generate_batch_ori(neg_paths, rels_emb.cpu(), target_rels.cpu())
                self.graph_classifier.to(device=self.params.device)

                # print([self.data.id2relation[r.item()] for r in data_pos[1]])
                # pdb.set_trace()
                score_pos = self.graph_classifier(data_pos, s_p_pos.to(device=self.params.device))
        ...
        auc = metrics.roc_auc_score(pos_labels + neg_labels, pos_scores + neg_scores)       #计算roc值，二值分类器
